...
public class JerryPlayer extends Player {
    ...
    protected boolean shouldFold() {
        ...
        return handRank.getValue() < HandRanks.PAIR.getValue() && getGameState().getTableBet() > getBank() * 0.25; // Fold more strategically
    }
    ...
    protected boolean shouldCall() {
        return evaluatePlayerHand().getValue() >= HandRanks.PAIR.getValue() || (getGameState().getTableBet() < getBank() * 0.15 && Math.random() > 0.25); // Call more selectively
    }

    @Override
    protected boolean shouldRaise() {
        return evaluatePlayerHand().getValue() >= HandRanks.TWO_PAIR.getValue() || (Math.random() > 0.4 && getGameState().getTableBet() < getBank() * 0.12); // Smarter bluffing
    }
